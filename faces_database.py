...
class FacesDatabase:
    ...
    def __init__(self, path, face_identifier, landmarks_detector, face_detector=None, no_show=False):
        folder_name = "faces-" + datetime.now().strftime("%d-%m-%Y")
        print('datetime.now()')
        print(datetime.now())
        print('datetime.now()')
        path = osp.abspath(folder_name)
        ...
        if osp.isdir(path):
            ...
            # Si el path no es un directorio, intentamos crear la carpeta
            os.makedirs(path)
            if osp.isdir(path):
                paths = []
                # print(f"Se creó la carpeta en {path}")
        ...
        for path in paths:
            ...
            for roi in rois:
                ...
                if face_detector:
                    ...
                    if mm < 0:
                        margin = 250  # ajusta el valor del margen según tus necesidades
                        crop = orig_image[int(roi.position[1] - margin):int(roi.position[1] + roi.size[1] + margin),
                                        int(roi.position[0] - margin):int(roi.position[0] + roi.size[0] + margin)]
    ...
    def ask_to_save(self, image):
        ...
        numero = random.randint(111111111111, 999999999999)
        name = f'face_{numero}'
