...
# Configurar la región de interés (ROI)
roi_height = 450  # Altura de la región inferior
h, w = 700, 1920  # Tamaño del frame (ajusta según tus necesidades)
roia = (0, h - roi_height, w, h)
...
class FrameProcessor:
    ...
    def process(self, frame):
        frame_roi = frame[roia[1]:roia[3], roia[0]:roia[2]]
        orig_image = frame_roi.copy()
        results = model.predict(frame_roi, show=False, conf=0.70, stream=True)


        cv2.rectangle(frame, (roia[0], roia[1]), (roia[2], roia[3]), (0, 255, 0), 2)
        rois = []
      
        if results:
            rois = extract_bounding_boxes(results, frame_roi)      
        ...
        landmarks = self.landmarks_detector.infer((frame_roi, rois))
        face_identities, unknowns, all_list = self.face_identifier.infer((frame_roi, rois, landmarks))
